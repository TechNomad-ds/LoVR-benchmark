#!/bin/bash
# Run jina-clip-v2 evaluation (encode_video). Set DATA_ROOT and EXP_DIR if needed. Run from evaluate/.

EVAL_ROOT=$(cd "$(dirname "$0")" && pwd)
NUM_TASKS=$(nvidia-smi -L 2>/dev/null | wc -l)
RUN_NAME=jina-clip-v2
...
for (( ID=0; ID<NUM_TASKS; ID++ )); do
    ...
    CUDA_VISIBLE_DEVICES=$ID nohup python ${RUN_NAME}_test.py \
        --encode_video --num_chunks $NUM_TASKS --chunk_idx $ID --interval 30 \
